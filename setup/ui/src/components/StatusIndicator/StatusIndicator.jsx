    ...
    render() {
        ...
        if (circleElements.includes(statusType)) {
            animationElement = <div
                className={`circle ${animate || (animationComplete && animation !== 'heartbeat') ? animation : ''}`} />
        } else if (statusType === 'iot') {
            animationElement = (
                <div
                    className={`hexagon-wrapper ${animate || (animationComplete && animation !== 'heartbeat') ? animation : ''}`}>
