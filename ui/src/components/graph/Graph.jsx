...
export function NumericalGraph(props) {
    ...
    /* array of values for dataset, used to normalize domain */
    const dataVals = [];
    ...
    const formattedData = deviceData.map((datapoint) => {
        ...
        return {
            ...
            [dataType]: parseToPrecision(datapoint[dataType], 4)
        }
    });

    // rounds y-axis labels to one decimal place
    ...
    return (
        <VictoryChart className={className} padding={{top: 0, left: 50, bottom: 55, right: 25}}>
            ...
            <VictoryAxis dependentAxis domain={{y: yDomain}} tickFormat={toString} />
            <VictoryLine data={formattedData} x="time" y={dataType} style={{data: {stroke: '#2842f4'}}} interpolation="monotoneX" />
        </VictoryChart>
    )
}
